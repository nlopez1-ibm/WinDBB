 ...
 */
def getLastGitTag(String gitDir) {
	String cmd = "git -C $gitDir describe --abbrev=0"
	StringBuffer gitLastTag = new StringBuffer()
	StringBuffer gitError  = new StringBuffer()
	
	Process process = cmd.execute()
	process.waitForProcessOutput(gitLastTag, gitError) 	
	if (gitError) {
		print("*! Error executing Git command: $cmd error: $gitError")
	}

	LastTag = gitLastTag.toString().trim()
	println "** --impactBuild build from last Git tag=" + LastTag

	// WHAT TO DO IF THERE IS NOT tag????
	
	cmd = "git -C $gitDir  rev-list -n 1 " + LastTag 
	StringBuffer gitLastTagCommit = new StringBuffer()
	process = cmd.execute()
	process.waitForProcessOutput(gitLastTagCommit, gitError) 	

	return gitLastTagCommit.toString().trim()
}
