...
def createCompileCommand(String buildFile, LogicalFile logicalFile, String member, File logFile) {
	...
	//njl concat prod libs 	 
	if(props.prod_copy) 
		compile.dd(new DDStatement().dsn(props.prod_copy).options("shr"))
	if (props.prod_BMScopy)
		compile.dd(new DDStatement().dsn(props.prod_BMScopy).options("shr"))
...
def createLinkEditCommand(String buildFile, LogicalFile logicalFile, String member, File logFile) {
    ...
    if (buildUtils.isCICS(logicalFile))
     ...
     // Add prod App Load Lib
	if (props.prod_load )
		linkedit.dd(new DDStatement().dsn(props.prod_load).options("shr"))
